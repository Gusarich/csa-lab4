...
The tracked design ground truth is [`ARCHITECTURE.md`](ARCHITECTURE.md). The
DataPath and Control Unit schemes are submitted as polished draw.io files in
[`docs/`](docs/), with committed SVG previews.
...
- `ARCHITECTURE.md` - complete tracked design document and architectural
  checklist.
...
- `python/golden/` - self-contained golden scenarios for binary output,
  listings, trace fragments, trap behavior, cache timing, and addressing corner
  cases.
- `docs/datapath.drawio` - submitted DataPath scheme.
- `docs/control_unit.drawio` - submitted Control Unit scheme.
- `docs/schemes.md` - short scheme review notes with SVG previews.
...
coverage report. The Markdown workflow checks tracked repository documentation
while ignoring the local `TASK/` handoff directory.
