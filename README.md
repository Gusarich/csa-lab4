...
Файлы схем находятся в репозитории:

* `docs/datapath.drawio` — DataPath;
* `docs/control_unit.drawio` — Control Unit.
...
```bash
pytest -v tests/golden --update-goldens
```
...
Исходный код реализации находится в `python/`. Основные точки входа:

* `python/asm.py` — CLI ассемблера;
* `python/machine.py` — CLI модели процессора;
* `python/assembler.py` — транслятор asm в бинарный образ, листинг и source map;
* `python/control_unit.py` — hardwired Control Unit;
* `python/datapath.py` — пассивный DataPath;
* `python/cache.py` — единый direct-mapped write-back кеш;
* `python/io_device.py` — trap-ввод и port-mapped вывод;
* `python/examples/` — обязательные asm-программы;
* `python/golden/` — самодостаточные golden YAML-сценарии;
* `docs/datapath.drawio` и `docs/control_unit.drawio` — схемы DataPath и Control Unit в формате draw.io.
...
```bash
uv run python asm.py examples/hello.asm out/hello.bin   --debug out/hello.hex   --map out/hello.map.json
touch out/empty.input
uv run python machine.py out/hello.bin out/empty.input   --map out/hello.map.json   --log out/hello.trace   --max-ticks 1000000
```
...
CI на GitHub Actions запускает форматирование, Ruff, mypy, unit-тесты, golden-тесты и проверку Markdown-документации на self-hosted runner.
